...
import { resetKeepOnDeath } from "../utils/remember-inventory"
...
export async function initModules(player) {
  ...
  try {
    const originModule = await import(`../data/origins/${playerOrigin}.js`);
    const originData = originModule[playerOrigin];

    POWERS.push(..._IMPORT_ORIGIN.powers, ...(originData.powers || []));
    PERKS.push(..._IMPORT_ORIGIN.perks, ...(originData.perks || []));
    CONTROLS.push(..._IMPORT_ORIGIN.controls, ...(originData.controls || []));
    EFFECTS = { ..._IMPORT_ORIGIN.effects, ...originData.effects };

  } catch (e) {
    console.warn(`[OriginsPE] Failed to load Origin: '${playerOrigin}' for ${player.name}`);
    console.warn(e);
    player.addTag('load_failed');
  }

  try {
    const classModule = await import(`../data/classes/${playerClass}.js`);
    const classData = classModule[playerClass];

    POWERS.push(..._IMPORT_CLASS.powers, ...(classData.powers || []));
    PERKS.push(..._IMPORT_CLASS.perks, ...(classData.perks || []));
    CONTROLS.push(..._IMPORT_CLASS.controls, ...(classData.controls || []));

  } catch (e) {
    console.warn(`[OriginsPE] Failed to load Class: '${playerClass}' for ${player.name}`);
    console.warn(e);
    player.addTag('load_failed');
  }

  POWERS.forEach(power => player.addTag(`power_${power}`));
  PERKS.forEach(perk => player.addTag(`perk_${perk}`));
  CONTROLS.forEach(control => player.addTag(`control_${control}`));
...
//   console.warn(`[Damage Report] ${hurtEntity.nameTag || hurtEntity.typeId} took ${damage} damage`);
// });
